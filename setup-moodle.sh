...
# Moodle Auto-Setup Script
# This script automatically sets up Moodle database if it doesn't exist
...
LOCK_FILE="/tmp/moodle-setup.lock"
...
if [ -f "$LOCK_FILE" ]; then
    info "Moodle setup already running (lock file exists)"
    exit 0
fi
